...
function titleSpecificResult(title: string, value: string, second = "") {
  ...
  // Common command/config generators.
  if (t.includes("docker compose") || t.includes("docker run")) {
  ...
  if ((t.includes("mean squared") || t.includes("rmse") || t.includes("mean absolute")) && n.length >= 2) {
    const h=Math.floor(n.length/2), a=n.slice(0,h), b=n.slice(h,h*2); const mse=a.reduce((s,x,i)=>s+(x-b[i])**2,0)/a.length; const mae=a.reduce((s,x,i)=>s+Math.abs(x-b[i]),0)/a.length;
    if (t.includes("rmse")) return `RMSE: ${Math.sqrt(mse).toFixed(6)}`; if (t.includes("mean absolute")) return `MAE: ${mae.toFixed(6)}`; return `MSE: ${mse.toFixed(6)}`;
  }
  ...
  // Final title-aware fallback: still a usable reference, never a fake success.
  return `# ${title}\nInput: ${safe(v)}\n\nReference result:\nUse the tool's named operation with the supplied input. Validate the generated value against the target runtime/specification before production use.`;
}

export function enhanceGenericDynamicTool(tool: DynamicTool): DynamicTool {
  return {
    ...tool,
    run: (value: string, value2?: string) => titleSpecificResult(tool.title, value, value2),
  };
}
